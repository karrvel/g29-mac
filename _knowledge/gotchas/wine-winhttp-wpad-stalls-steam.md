...
Verified fixed to the extent this fault goes: Steam self-updated to a full 1.6 GB client. It does **not** follow that Steam works — a re-test on 2026-09-02 showed the startup manifest request still burning its full 120-second timeout and then failing with `http error 0` before the client carries on regardless, so treat this as cosmetic slowness rather than a solved problem. What actually blocks Steam is a different fault entirely, in CEF: [[steam-webhelper-restart-loop-in-wine]]. An earlier version of this note claimed the client "reaches the login screen"; it does not. See [[raceroom-needs-steam-login]].
