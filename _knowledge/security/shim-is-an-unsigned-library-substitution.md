...
- **Only ever install a shim you built from `sdl2-lg4ff-shim.c` in this repo** via `make`. Never drop in a prebuilt `libSDL2-2.0.0.dylib` from anywhere else — the filename is the whole attack surface, and a hostile one would be loaded by every bottle with no prompt.
- **Verify state before trusting it:** `scripts/install-shim.sh status` should report the shim live and the original preserved as `libSDL2-2.0.0.real.dylib`. If the original is missing, the revert path is gone — restore from `libSDL2-2.0.0.dylib.orig-backup` or reinstall Whisky.
...
**Revert cleanly at any time:** `scripts/install-shim.sh revert` restores the original byte-for-byte and re-signs it. Do that before uninstalling or upgrading Whisky, so its updater never sees a substituted library.
